...
export default function LeaderboardPage() {
  return (
    <div className="space-y-6 max-w-4xl mx-auto animate-fade-up">
      <div>
        <h1 className="text-2xl font-bold text-white tracking-tight">🏆 訓練師<span className="text-gradient">排行榜</span></h1>
        <p className="text-white/40 text-sm mt-1.5">依本月 Agent 市場收益排名 · 每月 1 日更新</p>
      </div>

      {/* Top 3 podium */}
      <div className="gradient-ring overflow-hidden">
        <div className="relative p-7">
          <div className="absolute -top-16 -right-10 w-56 h-56 rounded-full bg-yellow-500/15 blur-3xl pointer-events-none" />
            ...
            {[BOARD[1], BOARD[0], BOARD[2]].map((p, i) => {
              ...
              return (
                <div key={p.rank} className="flex flex-col items-center gap-2">
                  ...
                  <div className={`${heights[i]} w-24 rounded-t-xl flex items-center justify-center text-3xl ${i === 1 ? "bg-gradient-to-br from-yellow-400/30 to-orange-400/20 border border-yellow-400/30 glow-soft" : "card"}`}>
                    {MEDALS[ranks[i] - 1]}
                  </div>
      ...
      <div className="card p-6">
        <div className="font-semibold text-white mb-4 flex items-center gap-2">📈 我的晉升路徑</div>
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 text-center">
            ...
            <Link href="/training" className="btn-primary inline-block text-xs px-4 py-2 rounded-xl font-semibold">訓練 Agent 上架</Link>
          </div>
        </div>
      </div>
    </div>
