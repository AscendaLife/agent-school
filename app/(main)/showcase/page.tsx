export default function ShowcasePage() {
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white">🏆 成果展示</h1>
        <p className="text-white/40 text-sm mt-1">學員訓練完成的 Agent 實戰成果展示</p>
      </div>
      <div className="grid grid-cols-2 gap-5">
        {works.map((w, i) => (
          <div key={i} className="rounded-xl bg-white/[0.04] border border-white/10 p-5 hover:border-purple-500/30 transition-all">
            <div className="flex items-start gap-3 mb-3">
              ...
              <span className="ml-auto text-xs px-2 py-0.5 rounded-full bg-purple-500/10 text-purple-400 border border-purple-500/20">{w.type}</span>
            </div>
            <p className="text-sm text-white/50 leading-relaxed mb-3">{w.desc}</p>
            <div className="flex flex-wrap gap-1 mb-3">
              {w.skills.map(s => <span key={s} className="text-xs px-2 py-0.5 rounded-full bg-white/5 text-white/40">{s}</span>)}
            </div>
            <div className="flex items-center justify-between pt-3 border-t border-white/5">
              <button className="text-xs text-white/30 hover:text-white/60 transition-all">❤️ {w.likes}</button>
              <button className="text-xs px-3 py-1.5 rounded-lg bg-white/5 text-white/40 hover:bg-white/10 transition-all">查看詳情</button>
            </div>
          </div>
        ))}
      </div>
